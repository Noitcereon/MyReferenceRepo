// TODO: delete Post  interface (below).
// Temporary interface while using placeholder api.
import { IPlayer } from "../models/IPlayer";
import { computed } from "vue";
import { ICommonResponse } from "../models/ICommonResponse"

const biteCodeModule = {
  namespaced: true,
  state: { 
    player: {
      playerId: Number,
  ...
  actions: {
    ...
    */
    async addKillInGame(context: any, killData: any) {
      ...
      await fetch(`${apiURL}/api/game/${killData.gameId}/player` , {
      method: "GET",
        headers: {
          //'X-API-Key': apiKey,
      ...
      await fetch(`${apiURL}/api/game/${killData.gameId}/kill`, {
        method: "POST",
        headers: {
          //'X-API-Key': apiKey,
          "Authorization": "Bearer " + killData.keycloakToken,
